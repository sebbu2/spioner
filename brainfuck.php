		...
		switch($s[$_s]) {
         ...
         if( (int)ord( $d[$_d] ) == 0 ) {
              ...
              to the $pos(ition) where the opening [ is. */
					if( brainfuck_interpret( $s, $_s, $d, $_d, $i, $_i, $o ) ) $_s = $pos;
				}
				break;
...
function brainfuck_regex_callback($value) {
	return str_repeat( $value[2], $value[0] );
}

function _brainfuck($source, $input = '') {
	$code = preg_replace_callback( '/(\d+)([><+-.,\[\]])/', 'brainfuck_regex_callback', $source );
